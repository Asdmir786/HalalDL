...
let cachedAria2InPathAvailable: boolean | null = null;

async function isAria2Available(aria2Path: string): Promise<boolean> {
  if (aria2Path !== "aria2c") return true;
  if (cachedAria2InPathAvailable !== null) return cachedAria2InPathAvailable;
  try {
    const cmd = Command.create("aria2c", ["--version"]);
    const output = await cmd.execute();
    cachedAria2InPathAvailable = output.code === 0;
  } catch {
    cachedAria2InPathAvailable = false;
  }
  return cachedAria2InPathAvailable;
}
...
export async function startDownload(jobId: string) {
  ...
  const aria2Available = await isAria2Available(aria2Path);
  if (aria2Available) {
    addLog({
      level: "info",
      message:
        aria2Path === "aria2c"
          ? "Using aria2c from system PATH"
          : `Using local aria2c: ${aria2Path}`,
      jobId,
    });
    ...
    addLog({
      level: "info",
      message: "aria2c not available; using yt-dlp built-in downloader",
      jobId,
    });
  }
  ...
  const filenameTemplate =
    job.overrides?.filenameTemplate || "%(title)s.%(ext)s";
  ...
  args.push("--print", "before_dl:__HALALDL_SIZE__:%(filesize_approx)s");
  ...
  try {
    ...
    const flushStdoutLine = (line: string) => {
      ...
      const hasFrequentUpdates =
        ...
        typeof update.speed === "string" ||
        typeof update.eta === "string" ||
        typeof update.totalSize === "string";
    ...
    const flushStderrLine = (line: string) => {
      ...
      if (
        trimmedLine.includes("aria2c") ||
        trimmedLine.includes("downloader")
      ) {
        addLog({
          level: "error",
          message: "Downloader specific error detected",
          jobId,
        });
      }
    ...
    cmd.on("close", (data) => {
      ...
      if (data.code === 0) {
        ...
        setTimeout(() => {
          const { settings } = useSettingsStore.getState();
          const finalJob = useDownloadsStore
            .getState()
          ...
          if (
            lastKnownOutputPath &&
            finalJob?.outputPath !== lastKnownOutputPath
          ) {
          ...
          if (outputPathToUse) {
            invoke<number>("get_file_size", { path: outputPathToUse })
              .then((bytes) => {
                if (Number.isFinite(bytes) && bytes > 0) {
                  updateJob(jobId, { totalSize: formatBytes(bytes) });
                }
              })
              .catch(() => void 0);
          }
    ...
    cmd.on("error", (error) => {
      ...
      if (message.toLowerCase().includes("invalid utf-8 sequence")) {
        addLog({
          level: "warn",
          message: `Process output decode warning: ${message}`,
          jobId,
        });
        return;
      }
  ...
  try {
    ...
    if (message.toLowerCase().includes("invalid utf-8 sequence")) {
      useLogsStore
        ...
        .addLog({
          level: "warn",
          message: `[meta] Metadata decode warning: ${message}`,
          jobId,
        });
      return;
    }
    useLogsStore
      ...
      .addLog({
        level: "error",
        message: `[meta] Failed to fetch metadata: ${message}`,
        jobId,
      });
  }
}
