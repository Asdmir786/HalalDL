    ...
    <div className="pt-2 border-t border-muted/50 grid gap-4 animate-in fade-in slide-in-from-top-2 duration-200">
      ...
      {!isCustomPreset && (
        <div className="flex items-center gap-2 bg-blue-500/10 text-blue-500 px-3 py-2 rounded-md text-xs border border-blue-500/20">
          ...
          <span>
            <strong>Preset Mode Active:</strong> Format settings are managed by
            the selected preset.
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="grid gap-2">
          <label className="text-xs font-medium text-muted-foreground">
            Output Format
            {!isCustomPreset && (
              <span className="ml-1 opacity-50">(Locked by Preset)</span>
            )}
          </label>
          <Select
            value={outputFormat}
          ...
          >
            <SelectTrigger
              className={cn(
                "h-9 bg-background/50 text-xs",
                !isCustomPreset && "opacity-70 cursor-not-allowed"
              )}
            >
        ...
        <div className="grid gap-2">
          ...
          <div className="flex gap-2">
            <Input
              readOnly
              value={
                customDownloadDir || defaultDownloadDir || "No default set"
              }
              className={cn(
