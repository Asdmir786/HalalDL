...
export function UpgradePrompt() {
  ...
  const hasMissingRequired = tools.some(
    (t) => t.status === "Missing" && t.required
  );
  ...
  const totalSize = selectedTools.reduce(
    (acc, id) => acc + (TOOL_SIZES[id] || 0),
    0
  );
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      try {
        if (!isTauriEnv) throw new Error("Not running in Tauri");
        const { invoke } = await import("@tauri-apps/api/core");
        await invoke("add_to_user_path");
      } catch (error) {
        addLog({
          level: "warn",
          message: `Failed to add to PATH: ${String(error)}`,
        });
      }
      if (results.some(Boolean)) {
        try {
          localStorage.setItem(
            "halaldl:pendingToolCongrats",
      ...
      addLog({
        level: "error",
        message: `Failed to finish setup: ${String(error)}`,
      });
                    ...
                    {tools.map((tool) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isMissing && (
                            <div
                              className={`w-2 h-2 rounded-full ${isSelected ? "bg-primary shadow-[0_0_8px_rgba(var(--primary),0.8)]" : "bg-muted-foreground/30"}`}
                            />
                          )}
                        </div>
                      );
              ...
              <>
                {!isMandatory && (
                  <MotionButton
                    ...
                    className="flex-1 h-11 rounded-xl text-muted-foreground hover:text-foreground"
                  >
                    Skip
                  </MotionButton>
                )}
